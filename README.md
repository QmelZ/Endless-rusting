...
## About
  Endless Rusting was originally made because I wanted to try learn Java, but now I'm continuing to work on it because I find it fun. The mod is about fleshing out the lore of PTER, and adding new mechanics which fit with vanilla.

## V7 Compatability
I've got plans for full v7 compatibility, though I'm not sure what's to come. Units are now compatable, so is the dialog, but I've got to wait and see if everything works for the next BE build.
...
Note: Preferably use the plain JAR for Windows. If you're compiling for an Android user, build the dexed JAR instead. Dexed JARs tend to be much larger, and are unnecessary if you can compile a plain JAR.
...
### Pulse
A weird substance which is simulated by Mindustry, and is collectible from the air.
...
At the moment, there's only one unit which is planned to have these, with two types of mounts and no weapons.
...
Slows the unit down drastically, and draws a spiky crystal around it. That's about it.
...
## Where's Pixelcraft's regular update scedule gone?
